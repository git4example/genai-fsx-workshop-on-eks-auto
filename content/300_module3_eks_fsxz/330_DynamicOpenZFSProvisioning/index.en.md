...
SUBNET_ID_1=$(aws ec2 describe-subnets --filters Name=vpc-id,Values=$VPC_ID --region $REGION_1 --query "Subnets[?AvailabilityZone=='${REGION_1}a'].SubnetId | [0]" --output text)
SUBNET_ID_2=$(aws ec2 describe-subnets --filters Name=vpc-id,Values=$VPC_ID --region $REGION_1 --query "Subnets[?AvailabilityZone=='${REGION_1}b'].SubnetId | [0]" --output text)
ROUTE_TABLE_1=$(aws ec2 describe-route-tables --filters "Name=vpc-id,Values=$VPC_ID" --query "RouteTables[?Associations[*].Main != 'true'] && RouteTables[].Associations[?SubnetId=='${SUBNET_ID_1}'].RouteTableId" --region $REGION_1 --output text)
ROUTE_TABLE_2=$(aws ec2 describe-route-tables --filters "Name=vpc-id,Values=$VPC_ID" --query "RouteTables[?Associations[*].Main != 'true'] && RouteTables[].Associations[?SubnetId=='${SUBNET_ID_2}'].RouteTableId" --region $REGION_1 --output text )
SECURITY_GROUP_ID=$(aws cloudformation describe-stacks --stack-name fsxZ-SecurityGroup --region $REGION_1 --query "Stacks[0].Outputs[0].OutputValue" --output text)
FSX_KEY=$(aws kms list-aliases --region $REGION_1 --query 'Aliases[?AliasName==`alias/aws/fsx`].TargetKeyId' --output text)
:::
...
::code[cd /eks-fsx-workshop/eks/FSxZ/filesystem-maz]{language=bash showLineNumbers=false showCopyAction=true}
:::
:::tab{id="windows" label="Windows"}
::code[cd /c/Users/Administrator/Desktop/eks-fsx-workshop/eks/FSxZ/filesystem-maz]{language=bash showLineNumbers=false showCopyAction=true}

You should be in the current working directory as shown in the below screenshot
![windows_FSxZ](/static/images/Windows_FSxZ_filesystem.png)

:::
:::tab{id="cloud9" label="Cloud9"}
::code[cd /home/ec2-user/environment/eks-fsx-workshop/eks/FSxZ/filesystem-maz]{language=bash showLineNumbers=false showCopyAction=true}
:::
::::
